...
More than one programming library exists to support easy reading and writing of miniSEED data without knowing the details of the format, the "libmseed":https://seiscode.iris.washington.edu/projects/libmseed library is supported and used extensively by the EarthScope DMC.
...
A dataless volume is commonly used to populate a metadata database or in combination with miniSEED to convert time series to an alternate format (using the EarthScope DMC's  "rdseed":/dms/nodes/dmc/software/downloads/rdseed/ software).
