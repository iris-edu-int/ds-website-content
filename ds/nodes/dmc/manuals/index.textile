...
The EarthScope DMC provides manuals for all software, tools and services we provide.
...
|"MATLAB Data Access":/dms/nodes/dmc/software/downloads/iris-ws/ |Waveform data, event metadata, and station metadata from the EarthScope-DMC can now be accessed within MATLAB using the IRIS-WS Java library. No knowledge of Java is required.|
|"MetaData Aggregator":/mda/help |The MetaData Aggregator (MDA) prints metadata summaries from the EarthScope DMC database including data availability and links to further services.|
...
** "FetchResp":/manuals/fetchresp.htm : Java client which will download response information from EarthScope DMC and construct RESP files on the user's computer.
...
table(table table-bordered table-striped).
|_. Name|_. Summary|
|"Seismic Analysis Code (SAC)":/files/sac-manual/ |Developed at Lawrence Livermore National Labs (LLNL) and distributed free by the EarthScope DMC|
